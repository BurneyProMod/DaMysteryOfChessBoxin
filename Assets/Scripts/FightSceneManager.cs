...
public class FightSceneManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (thing <= -1 && GameManager.FightType == true)
        {
            SceneManager.LoadScene("Startegy_Scene");
        }
        if (thing <= -1 && GameManager.FightType == false && (GameManager.fighters[GameManager.team1fighterIndex].health < GameManager.fighters[GameManager.team2fighterIndex].health))
        {
            SceneManager.LoadScene("WinPlayerTwo");
        }
        else if (thing <= -1 && GameManager.FightType == false && (GameManager.fighters[GameManager.team2fighterIndex].health < GameManager.fighters[GameManager.team1fighterIndex].health))
        {
            SceneManager.LoadScene("WinPlayerOne");
        }

        if (GameManager.fighters[GameManager.team1fighterIndex].health <= 0)
        {
            thing = 4;
            if (GameManager.fighters[0].health <= 0)
            {
        ...
        if (GameManager.fighters[GameManager.team2fighterIndex].health <= 0)
        {
            thing = 4;
            if (GameManager.fighters[4].health <= 0)
            {
